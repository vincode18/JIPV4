...

{
  ...
  key Resv.ReservationCategory,
      ...
      -- Priority: GI > NPB > RECEIVED > TR_REQUEST > PENDING
      -- Use date comparison to pick the LATEST milestone reached,
      -- because joins match by Plant+Material (not by Work Order),
      -- so multiple milestones from different WOs can co-exist.
      case
        ...
        -- NPB only if its date is later than RECEIVED date (or RECEIVED doesn't exist)
        when PM.WarehouseNumber is not null and EWM_NPB.MaterialNumber is not null
          and ( EWM_RCV.MaterialNumber is null or EWM_NPB.ConfirmedAt >= EWM_RCV.ConfirmedAt )
          then 'NPB'
        ...
        -- NPB only if its date is later than RECEIVED date (or RECEIVED doesn't exist)
        when PM.WarehouseNumber is null and WM_NPB.MaterialNumber is not null
          and ( WM_RCV.MaterialNumber is null or WM_NPB.ConfirmationDate >= WM_RCV.ConfirmationDate )
          then 'NPB'
